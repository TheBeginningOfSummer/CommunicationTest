...
namespace MyToolkit
{
    public class ConnectionToolkit
    {
        ...
        public class SocketConnection
        {
            ...
            #region 客户端
            public bool Connection(IPAddress ip, int port, out string error)
            {
                try
                {
                    IAsyncResult result = SocketItem.BeginConnect(ip, port, null, null);
                    bool isConnect = result.AsyncWaitHandle.WaitOne(TimeSpan.FromSeconds(3));
                    //SocketItem.Connect(ip, port);
                    if (!isConnect)
                    {
                        error = "连接超时";
                        return false;
            ...
            public bool Disconnection(out string error)
            {
                try
                {
            ...
            public void SendUTF8(string data)
            {
                ...
                //SendByte[2] = set1[1];
                //SendByte[3] = set1[0];
                SendByte = Encoding.UTF8.GetBytes(data);
        ...
        {
            //串口对象
            public readonly SerialPort MySerialPort;
            //ASCII码字符串
    ...
    public class ByteArrayToolkit
    {
        ...
        {
            List<int> index = new(sourceArray.Length);
            for (int i = 0; i < sourceArray.Length; i++)
            {
        ...
        public static byte[] LoopRemove(byte[] sourceArray, List<int> index, int count)
        {
            ...
            {
                return RemoveByte(LoopRemove(sourceArray, index, count - 1), index[^count]);
            }
        }
        #endregion
